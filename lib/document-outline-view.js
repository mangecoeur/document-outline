...
export default class DocumentOutlineView {
  ...
  highlightSectionAtCursor(cursorPos) {
    ...
    // _doHighlighSection(this.element, allItems, cursorPos, doScroll);
    let range;
    let item;
...
function _doHighlighSection(rootElement, allItems, cursorPos, doScroll) {
  // Attempt to avoid deopt issue
  let range;
  let item;
  for (item of allItems) {
    range = item.range;
  ...
  if (doScroll) {
    for (item of allItems) {
      ...
      if (range) {
        if (range.containsPoint(cursorPos)) {
          rootElement.scrollTop = item.offsetTop - 20;
          break;
        }
      }
    }
