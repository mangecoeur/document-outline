...
export default {
  config: {
    ...
    highlightCurrentSection: {
      type: 'boolean',
      default: true,
      description: 'Highlight the currently edited section in the outline'
    }
  ...
  activate() {
    ...
    atom.config.observe('document-outline.highlightCurrentSection', newValue => {
      if (newValue && this.docModel && this.view) {
  ...
  updateView() {
    if (this.view) {
      ...
      if (headingUpdates) {
        this.view.setModel(headingUpdates);
      }
    }
    // this.view.highlightSectionAtCursor(this.editor.getCursors()[0]);
  },
